...
def plot_loss_decompose(file_save, file, title_graph):
    ...
    plt.plot(epochs, test_pde, label='Loss pde test', linestyle='--', color='cyan')
    plt.plot(epochs, test_data, label='Loss data test', linestyle='--', color='lime')
    plt.plot(epochs, test_border, label='Loss border test', linestyle='--', color='orange')
...
def plot_results(
    ...
    checkpoint = torch.load('results/'+file_name+'/'+epoch+'/'+'model_weights.pth', map_location=torch.device('cpu'))
    ...
    p_pred =( p_norm_pred*mean_std['p_std'] + mean_std['p_mean'])*((param_adim['V']**2) * param_adim['rho'])
    ...
    # On plot les vidéos
    plot_flow(x_data, y_data, t_data, p_data, p_pred, name_file= "results/"+'/'+file_name+'/'+'result_pression.gif', title='Pression', fps=5)
    plot_flow(x_data, y_data, t_data, norme_vitesse_data, norme_vitesse_pred, name_file="results/"+'/'+file_name+'/'+'result_vitesse.gif', title='Vitesse', fps=5)
    print("OK les vidéos")

    # On plot la loss 
    plot_loss(file_save="results/"+file_name, file="results/"+file_name+'/'+epoch, title_graph=title_loss)
    print("OK le plot de la loss")

    # On plot la loss decompose 
    plot_loss_decompose(file_save="results/"+file_name, file="results/"+file_name+'/'+epoch, title_graph='loss_decompose')

    # On plot les points
    plot_points(X_train, X_border, mean_std, param_adim, file_save="results/"+file_name+'/points_coloc')
    print('OK pour les points de coloc')

    # On plot le Cl 
    plot_cl(X_full, U_full, model, param_adim, mean_std, file_save="results/"+file_name+'/courbe_cl')
